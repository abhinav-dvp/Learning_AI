(()=>{
  // Minimal frontend logic to fetch recommendations and render cards.
  const $ = sel=>document.querySelector(sel)
  const $$ = sel=>Array.from(document.querySelectorAll(sel))

  const topicsInput = $('#topicsInput')
  const initBtn = $('#initBtn')
  const refreshBtn = $('#refreshBtn')
  const recommendationsEl = $('#recommendations')
  const shownCount = $('#shownCount')
  const poolSize = $('#poolSize')
  const poolVal = $('#poolVal')

  poolSize.oninput = ()=>poolVal.textContent = poolSize.value

  initBtn.onclick = async ()=>{
    const topics = topicsInput.value.split('\n').map(s=>s.trim()).filter(Boolean)
    if(!topics.length) return alert('Add at least one topic')
    await fetch('/api/config', {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({topics})})
    refresh()
  }
  ...
  async function refresh(){
    recommendationsEl.innerHTML = '<div class="muted">Loading…</div>'
    try{
      const resp = await fetch('/api/recommend?limit='+poolSize.value)
      const data = await resp.json()
      render(data || [])
    }catch(e){
      recommendationsEl.innerHTML = '<div class="muted">Error fetching</div>'
      console.error(e)
    }
  }
  ...
  function render(items){
    ...
    recommendationsEl.innerHTML = items.map(it=>`<div class="recommendation card"><h3>${escapeHtml(it.title||it.name||'Untitled')}</h3><p>${escapeHtml((it.summary||it.description||'').slice(0,240))}</p><div style="height:8px"></div><div style="display:flex;gap:8px"><a class="btn ghost small" href="${it.url||'#'}" target="_blank">Open</a></div></div>`).join('\n')
  }
