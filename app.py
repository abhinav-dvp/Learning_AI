...
class CourseDiscovery:
    ...
    def search_courses(self, query, max_results=5):
        logger.info(f"Searching courses: {query}")
        query_lower = query.lower()

        matching = []
        for course in self.courses:
            if any(keyword in course['title'].lower() or 
...
class EnhancedSystem:
    ...
    def daily_discovery(self, topics):
        ...
        for topic in topics[:3]:
            ...
            courses = self.course_discovery.search_courses(topic, Config.MAX_COURSES_PER_SEARCH)
